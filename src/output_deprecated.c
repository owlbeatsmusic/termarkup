...
void output_format_token_to_fit(Token *token, char *before, char *after, int non_ascii_offset, Bool multiple_lines_bool, Bool fit_to_full_width) {
	...
	// creating the padding before and after each line
	...
	for (int i = 0; i < 2; i++) {
        full_width_padding[i] = (char*)malloc((padding_sizes[i] + 1) * sizeof(char));
